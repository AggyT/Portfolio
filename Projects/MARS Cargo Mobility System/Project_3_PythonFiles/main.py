...
# and execution.
# 
# This program is version of main used for the MACRO demonstration.
# 
# 
...
BP.set_sensor_type(BP.PORT_3, BP.SENSOR_TYPE.CUSTOM, [(BP.SENSOR_CUSTOM.PIN1_ADC)]) # Configure for an analog on sensor port pin 1, and pull the analog line on pin 1.
...
def main(site):
    magTime = time.time() # Time that the last magnet beacon was detected
    integral = 0
    derivative = 0
    magcount = 0
    magnet = 0
    ...
    iop3.siteArrival(magcount)
    Turns.rightTurn(left, right)
    magnet = 0
    ...
    while (True):
        ...
        if (left == 1 and right == 1):
            ...
            while (grovepi.digitalRead(k.RIGHT) == 0):
                BP.set_motor_power(BP.PORT_A, 0 * k.POWER)
                BP.set_motor_power(BP.PORT_D, k.POWER / 2)
                time.sleep(k.SLEEP)
    ...
    while True:
        ...
        if (left == 1 and right == 1):
            while (grovepi.digitalRead(k.LEFT) == 1): 
                BP.set_motor_power(BP.PORT_A, k.POWER)
                time.sleep(k.SLEEP)
            ...
            while (grovepi.digitalRead(k.LEFT) == 0):
                BP.set_motor_power(BP.PORT_A, k.POWER / 2)
                BP.set_motor_power(BP.PORT_D, k. 0)
                time.sleep(k.SLEEP)
